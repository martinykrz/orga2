...
#define ERASE_CHAR 7
static void erase_char(uint8_t);
...
static const struct scancode_info scancode_defs[256] = {
	...
	[0x3b] = { '\0', CHANGE_COLOR }, // F1
	[0x3c] = { '\0', CHANGE_COLOR }, // F2
	[0x3d] = { '\0', CHANGE_COLOR }, // F3
	[0x3e] = { '\0', CHANGE_COLOR }, // F4
	[0x3f] = { '\0', CHANGE_COLOR }, // F5
	[0x40] = { '\0', CHANGE_COLOR }, // F6
	[0x41] = { '\0', CHANGE_COLOR }, // F7
	[0x42] = { '\0', CHANGE_COLOR }, // F8
	[0x43] = { '\0', CHANGE_COLOR }, // F9
	[0x44] = { '\0', CHANGE_COLOR }, // F10
	...
	[0x57] = { '\0', CHANGE_COLOR }, // F11 on a 101+ key keyboard
	[0x58] = { '\0', CHANGE_COLOR } // F12 on a 101+ key keyboard
};

static int curr_row = 0;
static int curr_column = 0;
static int is_shift_pressed = 0;
static uint8_t curr_fg = C_FG_BLACK;
static uint8_t curr_bg = C_BG_RED;
...
static void erase_char(unused uint8_t scancode) {
    restore_marker();
    // Move back
    curr_column--;
    // Store current color attr
    uint8_t tmp_attr = curr_attr;
    curr_attr = C_FG_BLACK | C_BG_BLACK;
    // Any char -> ' '
    put(scancode_defs[0x39].main_value); 
    // Restore color attr
    curr_attr = tmp_attr;
    fix_rowcol();
    put_marker();
}
static void change_color(unused uint8_t scancode) {
    restore_marker();
    
    // Key -> Foreground Color -> Shift
    // F01 0x3b -> Black -> Dark Grey
    // F02 0x3c -> Blue -> Light Blue
    // F03 0x3d -> Green -> Light Green
    // F04 0x3e -> Cyan -> Light Cyan
    // F05 0x3f -> Red -> Light Red
    // F06 0x40 -> Magenta -> Light Magenta
    // F07 0x41 -> Brown -> Light Brown
    // F08 0x42 -> White -> Light Grey
    //
    // Key -> Background Color -> Shift
    // F09 0x43 -> Black -> Light Grey
    // F10 0x44 -> Blue -> Brown
    // F11 0x57 -> Red -> Green
    // F12 0x58 -> Cyan -> Magenta

    switch (scancode) {
        case 0x3b: //F1
            curr_fg = is_shift_pressed ? C_FG_DARK_GREY : C_FG_BLACK;
            break;
        case 0x3c: //F2
            curr_fg = is_shift_pressed ? C_FG_LIGHT_BLUE : C_FG_BLUE;
            break;
        case 0x3d: //F3
            curr_fg = is_shift_pressed ? C_FG_LIGHT_GREEN : C_FG_GREEN;
            break;
        case 0x3e: //F4
            curr_fg = is_shift_pressed ? C_FG_LIGHT_CYAN : C_FG_CYAN;
            break;
        case 0x3f: //F5
            curr_fg = is_shift_pressed ? C_FG_LIGHT_RED : C_FG_RED;
            break;
        case 0x40: //F6
            curr_fg = is_shift_pressed ? C_FG_LIGHT_MAGENTA : C_FG_MAGENTA;
            break;
        case 0x41: //F7
            curr_fg = is_shift_pressed ? C_FG_LIGHT_BROWN : C_FG_BROWN;
            break;
        case 0x42: //F8
            curr_fg = is_shift_pressed ? C_FG_WHITE : C_FG_LIGHT_GREY;
            break;
        case 0x43: //F9
            curr_bg = is_shift_pressed ? C_BG_LIGHT_GREY : C_BG_BLACK;
            break;
        case 0x44: //F10
            curr_bg = is_shift_pressed ? C_BG_BROWN : C_BG_BLUE;
            break;
        case 0x57: //F11
            curr_bg = is_shift_pressed ? C_BG_GREEN : C_BG_RED;
            break;
        case 0x58: //F12
            curr_bg = is_shift_pressed ? C_BG_MAGENTA : C_BG_CYAN;
            break;
    }
    
    curr_attr = curr_fg | curr_bg;
